...
impl ChatFakerModel {
    ...
    async fn load_from_file_async(_path: PathBuf) -> Result<Self, LLMEndpointError> {
        Ok(Self {})
    }
    ...
    async fn stream_chat_completions(
        ...
        Err(LLMEndpointError::Advance("test".to_string()))
    }
...
fn completions_for(prompt: &str) -> String {
    ...
    if prompt.contains("capital") {
        if prompt.contains("portugal") {
            return "The capital of Portugal is Lisbon.".to_string();
        } else {
            return "The capital of Canada is Ottawa.".to_string();
        }
    } else if prompt.contains("long") {
        return "A very long string".to_string();
    } else {
        return "The answer is 42".to_string();
    }
}

struct ChatFakerSession {}
...
/*
impl Drop for ChatFakerEndpoint {
    fn drop(&mut self) {
...
*/
/*
/// A [`ChatFakerModel`] (as well as its associated [`ChatFakerSession`]s) that unloads itself from memory after not being used
/// for a period of time.
struct UnloadingModel {
    model: Perishable<ChatFakerModel>,
    path: PathBuf,
    // sessions: Arc<DashMap<SessionId, Perishable<ChatFakerSession>>>,
    // maintenance_thread: JoinHandle<()>,
    // finished_tx: UnboundedSender<(SessionId, Perishable<ChatFakerSession>)>,
}
...
impl UnloadingModel {
    ...
    async fn new(model_path: impl AsRef<Path>) -> Self {
        let sessions: Arc<DashMap<SessionId, Perishable<ChatFakerSession>>> = Default::default();
    ...
    /// Either takes an existing chat [`ChatFakerSession`] compatible with the provided prompt from the
    ...
    async fn take_chat_session<'a>(
        &self,
        prompt: &'a str,
    ) -> (Perishable<ChatFakerSession>, SessionId, &'a str) {
...
/*
/// Helper function to acquire a read guard to a [`ChatFakerModel`] (and its associated
/// [`ActiveSignal`]).
async fn get_or_init_model(
    model: &Perishable<ChatFakerModel>,
    path: impl AsRef<Path>,
) -> Result<(ActiveSignal, PerishableReadGuard<ChatFakerModel>), LLMEndpointError> {
    let path = path.as_ref().to_path_buf();
    model
        .get_or_try_init(move || async move {
            info!("Loading {} into memory", path.to_string_lossy());

            ChatFakerModel::load_from_file_async(path)
                .await
...
/*
/// Helper function to acquire a write guard to a [`ChatFakerSession`] (and its associated
/// [`ActiveSignal`]).
async fn get_or_init_session(
    session: &Perishable<ChatFakerSession>,
    model: ChatFakerModel,
) -> Result<(ActiveSignal, PerishableWriteGuard<ChatFakerSession>), LLMEndpointError> {
    session
...
impl SessionId {
    ...
    /// [`SessionId`] and "stored" in the corresponding [`ChatFakerSession`].
...
struct CompletionStream {
    ...
    finished_tx: Option<UnboundedSender<(SessionId, Perishable<ChatFakerSession>)>>,
...
impl CompletionStream {
    ...
    /// * `model` - The [`ChatFakerModel`] that `session` is associated with.
    ...
    async fn new(
        ...
        let (session_signal, handle) = {
            ...
            (
                session_signal,
                session_guard.start_completing_with(SINGLE_MESSAGE_LIMIT),
            )
        };
...
#[derive(Default)]
enum SessionOption {
    OneShot(ChatFakerSession),
    Perishable(Perishable<ChatFakerSession>),
    #[default]
    None,
}
